...
new_player = Player("Mosharraf", room["outside"], [])
# Write a loop that:
#
...
selection = input(
    'Please select a direction to move using n, s, e, w. You can add items to your inventory with g and drop them with d. If you want to quit, press q.\n')
while(selection != 'q'):
    if selection == 'g':
        if len(new_player.current_room.items) == 0:
            ...
            new_player.current_room.remove_item(
                new_player.current_room.items[0])
        ...
        print(new_player.current_room)
        selection = input(
            'Please select a direction to move using n, s, e, w, or quit with q:\n')
